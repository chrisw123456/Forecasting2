...
#data <- read_excel("FieldOpsMMVolumedatafromJan2019.xlsx") 

#colnames(data)[3] <- "Sub.Product"
...
data<-data %>%
     ...
     filter(n() >= 22) %>%
     ungroup
# data$Date <- paste(data$Date,"01",sep="") %>% as.character() %>%
#   lubridate::ymd()

# automate this so I delete anything with less than 20 observations

# data<-data[!(data$product_subproduct=="Professional Services_and_Change" | data$product_subproduct=="Professional Services_and_Change"),]
data %>%
   group_by(Scenario) %>%
...
#x<-split(data, data$Scenario) %>% global2env
...
md <- train_model(input = list_ts_s1[[3]],
                  methods = methods,
